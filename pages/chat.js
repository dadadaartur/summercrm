...
export default function ChatPage() {
  ...
  const refreshSessionStats = async () => {
    ...
    const { data } = await supabase
      .from('chat_sessions')
      .select('message_count, client_message_count, operator_message_count, auto_rating')
      .eq('id', sessionId)
      .single()
  ...
  const closeChat = async () => {
    ...
    try {
      ...
      if (durationMinutes > 5) {
        penaltyPoints += 1
        reasons.push('долгое ожидание')
      }

      if (session.message_count > 10) {
        penaltyPoints += 1
        reasons.push('много сообщений')
      }
      ...
      const { data: firstMsg } = await supabase
        ...
        .limit(1)
        .single()
      if (firstMsg?.sender_type !== 'operator') {
        penaltyPoints += 1
        reasons.push('оператор не ответил первым')
      }
      ...
      showNotification('Ошибка при закрытии чата')
    }
  }
    ...
    <>
      ...
      <div className="crm-topbar">
        ...
        <div className="topbar-right">
          <span className="topbar-name">{user?.email}</span>
        </div>
      </div>
      ...
      <div className="chat-container">
        <div className="chat-left-panel">
          ...
          <div className="chat-metrics">
            <div className="chat-metrics-item">
              ...
              <span>{sessionStats.message_count}</span>
            </div>
            <div className="chat-metrics-item">
              <span>От клиента</span>
              <span>{sessionStats.client_message_count}</span>
            </div>
            <div className="chat-metrics-item">
              <span>От оператора</span>
              <span>{sessionStats.operator_message_count}</span>
            </div>
            {sessionStats.auto_rating && (
              <div className="chat-metrics-item">
                <span>Автооценка</span>
                <span>{sessionStats.auto_rating}★</span>
              </div>
            )}
          </div>
        ...
        <div className="chat-center-panel">
          <div className="chat-messages">
            ...
            {messages.length === 0 && !loading && (
              <div style={{ textAlign: 'center', color: '#5B7465', marginTop: 40 }}>
        ...
        <div className="chat-right-panel">
          <div className="chat-tabs">
            <button
              className={`chat-tab ${rightTab === 'templates' ? 'active' : ''}`}
              onClick={() => setRightTab('templates')}
            >
              Шаблоны
            </button>
            <button
              className={`chat-tab ${rightTab === 'actions' ? 'active' : ''}`}
          ...
          {rightTab === 'actions' && (
            <div>
              ...
              <button className="chat-action-btn" style={{ background: '#F2F9F4', color: '#2D6A4F' }} onClick={() => showNotification('Функция в разработке')}>
                Назначить встречу
              </button>
              <button className="chat-action-btn" style={{ background: '#FEE2E2', color: '#B91C1C' }} onClick={closeChat}>
                Закрыть чат
              </button>
            </div>
          )}
